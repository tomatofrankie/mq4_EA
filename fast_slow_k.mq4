...
void CheckStochCross(){
   ...
   double curr_fast_k=iStochastic(Symbol(),0,fast_k,fast_d,fast_smoothing,MODE_SMA,STO_LOWHIGH,MODE_BASE,0);
   double curr_slow_k=iStochastic(Symbol(),0,slow_k,slow_d,slow_smoothing,MODE_SMA,STO_LOWHIGH,MODE_BASE,0);
   double pre1_fast_k=iStochastic(Symbol(),0,fast_k,fast_d,fast_smoothing,MODE_SMA,STO_LOWHIGH,MODE_BASE,1);
   double pre1_slow_k=iStochastic(Symbol(),0,slow_k,slow_d,slow_smoothing,MODE_SMA,STO_LOWHIGH,MODE_BASE,1);
   double pre2_fast_k=iStochastic(Symbol(),0,fast_k,fast_d,fast_smoothing,MODE_SMA,STO_LOWHIGH,MODE_BASE,2);
   //double pre2_slow_k=iStochastic(Symbol(),0,slow_k,slow_d,slow_smoothing,MODE_SMA,STO_LOWHIGH,MODE_BASE,2);
   
   if(curr_slow_k>LowerThreshold && pre1_slow_k<LowerThreshold && (curr_fast_k>LowerThreshold && (pre1_fast_k<LowerThreshold||pre2_fast_k<LowerThreshold))){
      CrossToOpenBuy=true;
   }
   if(curr_slow_k<UpperThreshold && pre1_slow_k>UpperThreshold && (curr_fast_k<UpperThreshold && (pre1_fast_k>UpperThreshold||pre2_fast_k>UpperThreshold))){
      CrossToOpenSell=true;
   }
   //if(curr_slow_k<UpperThreshold && pre1_slow_k>UpperThreshold && (curr_fast_k<UpperThreshold && (pre1_fast_k>UpperThreshold||pre2_fast_k>UpperThreshold))){
   if((curr_slow_k>UpperThreshold && pre1_slow_k<UpperThreshold)||(curr_slow_k<LowerThreshold && pre1_slow_k>LowerThreshold)){   
      CrossToCloseBuy=true;
   }
   //if(curr_slow_k>LowerThreshold && pre1_slow_k<LowerThreshold && (curr_fast_k>LowerThreshold && (pre1_fast_k<LowerThreshold||pre2_fast_k<LowerThreshold))){
   if((curr_slow_k<LowerThreshold && pre1_slow_k>LowerThreshold)||(curr_slow_k>UpperThreshold && pre1_slow_k<UpperThreshold)){   
