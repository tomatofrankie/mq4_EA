...
void CheckStochCross(){
   ...
   for(int i = 0;i < 64;i++) {
      ...
      //HeikinMedian[i] = Close[i];
   }
   
   //double jaw2 = iMA(Symbol(),0,13,8,MODE_SMMA,PRICE_MEDIAN,0);
   //double jaw3 = iMA(Symbol(),0,13,8,MODE_EMA,PRICE_MEDIAN,8);
   //double jaw4 = iMA(Symbol(),0,13,0,MODE_EMA,PRICE_MEDIAN,0);
   double jaw = iSMMA(HeikinMedian, 13, 8);
   double teeth = iSMMA(HeikinMedian, 8, 5);
   double lips = iSMMA(HeikinMedian, 5, 3);

   
   //Print(StringFormat("%G %G", jaw, jaw2));
