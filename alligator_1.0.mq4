...
void CheckStochCross(){
   ...
   for(int i = 0;i < 64;i++) {
      ...
      //HeikinMedian[i] = Close[i + 1];
   }
   
   
   //double teeth2 = iMA(Symbol(),0,15,0,MODE_EMA,PRICE_CLOSE,0);
   //double jaw2 = iMA(Symbol(),0,25,0,MODE_EMA,PRICE_CLOSE,0);
   ...
   isGreenHeiki(Symbol(),0,1)==True && isGreenHeiki(Symbol(),0,2)==True
   ){
      CrossToOpenBuy=true;
   }
   if(jaw>teeth && teeth>lips &&
   isRedHeiki(Symbol(),0,1)==True && isRedHeiki(Symbol(),0,2)==True
   ){
